...
from indico_payment_icbc3.blueprint import blueprint
from indico_payment_icbc3.util import RsaUtil, aes_encrypt
...
class ICBC3PaymentPlugin(PaymentPluginMixin, IndicoPlugin):
    """ICBCpay
    ...
    def adjust_payment_form_data(self, data):
        ...
        biz_content["mer_reference"] = urlparse(
            url_for_plugin(
                "payment_icbc3.notify", registration.locator.uuid, _external=True
            )
        ).hostname
        biz_content["mer_url"] = url_for_plugin(
            "payment_icbc3.success", registration.locator.uuid, _external=True
        )
